...
if [ $# -eq 0 ]; then
    ...
    files=$(git ls-files -- '*.c' '*.h' '*.cpp' '*.hpp' '*.cc' '*.hh' '*.cxx' '*.m' '*.mm' '*.inc' '*.java' '*.glsl' ':!:external/*', ':!:.github/*')
else
    # $1 should be a file listing file paths to process. Used in CI.
    files=$(cat "$1" | grep -v "external/" | grep -E "\.(c|h|cpp|hpp|cc|hh|cxx|m|mm|inc|java|glsl)$" | grep -v "platform/android/java/lib/src/com/google/" | grep -v "\-so_wrap\." | grep -v "tests/python_build/")
fi
